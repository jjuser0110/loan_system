...
class BackfillSkimASchedule extends Command
{
    protected $signature   = 'schedule:backfill-skim-a';
    protected $description = 'One-time backfill: create missing monthly schedules for SKIM A loans up to current month';

    public function handle(): void
    {
        $today = Carbon::today();

        $this->info("Backfilling SKIM A schedules — today: {$today->toDateString()}");
        ...
        if ($loans->isEmpty()) {
            $this->info('No eligible SKIM A loans found.');
            return;
        }
        ...
        foreach ($loans as $loan) {
            ...
            if (! $latestSchedule) {
                $this->warn("No schedule found for loan_code: {$loan->loan_code}, skipping.");
                $skipped++;
                continue;
            }
            ...
            // Already has a future schedule, nothing to backfill
        ...
        $this->info("Backfill done. Created: {$created}, Skipped: {$skipped}");
        Log::info("SKIM A Backfill — Created: {$created}, Skipped: {$skipped}, Date: {$today->toDateString()}");
    }
